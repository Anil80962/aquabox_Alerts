...
# 1. Install plymouth-themes for the script plugin
stdin, stdout, stderr = ssh.exec_command(
    'echo "aquabox@2026" | sudo -S apt-get install -y plymouth-themes 2>&1 | tail -3',
    timeout=60
)
time.sleep(10)
print(stdout.read().decode('utf-8', errors='replace'))

# 2. Set plymouth theme properly using full path
stdin, stdout, stderr = ssh.exec_command(
    'echo "aquabox@2026" | sudo -S /usr/sbin/plymouth-set-default-theme aquabox 2>&1'
)
time.sleep(2)
print('Set theme:', stdout.read().decode('utf-8', errors='replace').strip())

# Rebuild initramfs
stdin, stdout, stderr = ssh.exec_command(
    'echo "aquabox@2026" | sudo -S update-initramfs -u 2>&1',
    timeout=120
)
time.sleep(30)
print(stdout.read().decode('utf-8', errors='replace')[-200:])

# 3. Create a boot animation service that shows the Fluxgen water animation
# This runs the same display_logo.py but as a one-shot at boot before desktop
BOOT_ANIM = '''#!/usr/bin/env python3
...
sftp = ssh.open_sftp()
with sftp.file('/home/aquabox/Desktop/Aquabox/boot_splash.py', 'w') as f:
    f.write(BOOT_ANIM)
sftp.close()

# 4. Create systemd service for boot splash (starts before alerts)
BOOT_SERVICE = '''[Unit]
Description=AquaBox Boot Splash
After=graphical.target
Before=aquabox-alerts.service
...
print('Boot splash installed. Rebooting...')
