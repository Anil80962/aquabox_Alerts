...
def _make_time_readable(text, lang="en"):
    """Convert time like 02:16 PM to spoken words in the given language."""
    import re
...
def translate_alert_text(title, body, status, lang):
    ...
    # Make units and time readable in the target language
...
class AlertsWindow(Gtk.Window):
    ...
    def _do_logout(self):
        global USERNAME, PASSWORD, LOGGED_IN, token, token_time, _announce_stop, _auto_announcing
        _announce_stop = True
        _auto_announcing = False
        USERNAME = ""
        PASSWORD = ""
